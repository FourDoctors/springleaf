...
data, and not the columns itself.
...
```
The structure of the data in train returned by _str_ above is strings,
however we can use the function _class_ to obtain the type of each
column,
...
```
We will proceed with PCA later. For now we should look into the
_character_ data to see what those strings  might be! These variables
...
For any predictor with 10 or less distinct values, we can obtain their values and look at them
...
function to tell us if they are. To write this function we have
...
We begin to make a dictionary that labels the variables.
```{r varlabels}
varlabels <- list()
n = 1
