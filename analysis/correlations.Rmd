...
var.entropies <- predictor.entropies(predictors)
...
The variable _distinct_ in *kld_num_train_df* also counts NA. We will
create another variable in that data-frame that tells us if the
variable contains NAs, and will then add entropies, min, max, etc from _var.entropies_.
...
We will write a function that filters the predictors by their number
of NAs. To begin we explore how many NAs there are in the
variables. We will make a CDF plot.

```{r numnacdf}

nacount <- sort(var.entropies$nacount)
naprob <- (1:length(nacount))/length(nacount)
xyplot(naprob ~ log10(nacount), type='l')

```
From the plot we can see that about 85% of the predictors have fewer
than a thousand NAs, suggesting that we can drop the remaining
predictors. In fact for this data, the CDF plot was probably
unnecessary. We can just glare at the table,

```{r numnatable}

nacount_table <- data.frame(table(var.entropies[, c("datatype", "nacount")]))
nacount_table$nacount <- as.numeric(as.character(nacount_table$nacount))
print(nacount_table)

print("Number of predictors without any NAs")
print(with(nacount_table, Freq[nacount==0]))
print("Number of predictors with more than 100000 NAs")
print(with(nacount_table, sum(Freq[nacount > 100000])))

```
A first filter then should be 0 NAs,

```{r nafilter0}

print(subset(nacount_table, nacount==0))

```

We will miss the truly numeric variables in this filter!

```{r nonapredictors}

nonapredictors <- with(var.entropies, variable[nacount == 0])

```

#Numeric variables with many distinct values.

How many distinct values do the predictors have?
Or how many predictors with a few distinct values?

```{r distinctdistr}

dvs <- sort( var.entropies$distinct_vals)
histogram(~ log10(dvs))
dvp <- (1:length(dvs))/length(dvs)
xyplot(dvp ~ log2(dvs), type="l")

```

```{r entdistr}

ents <- sort(var.entropies$entropy)
histogram( ~ ents)
ents_p <- (1:length(ents))/length(ents)
xyplot(ents_p ~ ents, type="l")

```

In our comparison of variables we will need to compare two
variables. For this we write a function,
...
For a plot of histograms for variables that have a given number of
effective values we write a function,

```{r plotvarsgiveneffvals}
...
Lets give var.entropies columns for max, min, and median for non-ec
values.
...
var.entropies$max_nonec <- sapply(var.entropies$variable,
                                  function(p){
                                      max(removedValues(train[,p], ovl_10[[p]]),
                                          na.rm=TRUE)
                                  })
var.entropies$min_nonec <- sapply(var.entropies$variable,
                                  function(p){
                                      min(removedValues(train[,p], ovl_10[[p]]),
                                          na.rm=TRUE)
                                  })
var.entropies$median_nonec <- sapply(var.entropies$variable,
                                  function(p){
                                      median(removedValues(train[, p], ovl_10[[p]]),
                                             na.rm=TRUE)
                                  })

viewVarEnt <- function(vents=var.entropies){
    vents[, c('entropy', 'min_nonec', 'median_nonec', 'max_nonec', 'distinct_vals', 'nacount')]
}
