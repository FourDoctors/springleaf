...
prob.1.pzs <- do.call(rbind, lapply(pzs, function(p) {
                                        df <- train[, c('target', p)]
                                        names(df) <- c('target', 'variable')
                                        df$variable <- df$variable != 0
                                        df$variable[ is.na(df$variable) ] <- FALSE
                                        tdf <- table(df)
                                        print(tdf)
...
train.znz[ is.na(train.znz) ] <- FALSE
...
fit.rpart.znz <- rpart(target ~ ., data=train.znz, control=rpart.control(depth=100))
