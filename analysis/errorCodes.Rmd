...
```
Lets begin by investigating variables that take 3 distinct values,
...
really 2 valued. The third value is NA. For these, we can compute the
correlation matrix,
```{r threedistinctnum2}
...
write.csv(train.ec.fixed, file=paste(datapath, 'train_ecfixed.csv', sep=""))
...
write.csv(test.ec.fixed, file=paste(datapath, 'test_ecfixed.csv', sep=""))
```
