...
bstDence.cv <- xgb.cv(param = list(max.depth=9, eta=0.01, silent=1,
...
grid.xgb <- expand.grid(max.depth = c(6, 9),
                         eta = c(0.01, 0.1))
nrounds = 20

bstDence.tuning <- foreach(n=1:nrow(grid.xgb), .combine = 'rbind') %do% {
    md <- grid.xgb$max.depth[n]
    eta <- grid.xgb$eta[n]
    print(paste("max.depth", md, "eta", eta))
    bstDence.cv <- xgb.cv(param = list(max.depth = md,
                              eta = eta,
                              silent = 1,
                              nthread=3,
                              objective = 'binary:logistic'),
                          ...
                          nfold = 5,
                          metrics = {'auc'} )
    bstDence.cv$nrounds <- 1:nrounds
    bstDence.cv$max.depth <- md
    bstDence.cv$eta <- eta
    bstDence.cv
}
